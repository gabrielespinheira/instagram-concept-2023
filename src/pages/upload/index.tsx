...
const UploadPage: NextPage = () => {
  ...
  const [subtitle, setSubtitle] = useState('') // TODO: add validation with zod
  ...
  async function handleUpload(e) {
    ...
    if (file && file.length > 0 && session && session.token) {
      ...
      try {
        ...
        return router.push(`/upload/${json.post.id}`)
      } catch (error) {
  ...
  return (
    <BasicTemplate>
      <h1
        className="text-5xl md:text-7xl font-bold leading-tighter tracking-tighter"
        data-aos="zoom-y-out"
      >
