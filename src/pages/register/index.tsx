...
import { mutationAuthRegister } from 'graphql/sdk/auth'
import { RegisterSchema, RegisterType } from 'graphql/schemas'
...
const RegisterPage: NextPage = () => {
  ...
  const {
    ...
    resolver: validationResolver(RegisterSchema),
  })
  ...
  const onSubmit: SubmitHandler<RegisterType> = async (data) => {
    ...
    try {
      const resp = await fetcher(mutationAuthRegister, data)

      setSession({
        user: resp.register.user,
        token: resp.register.token,
        signed: !!resp.register.user,
      })
